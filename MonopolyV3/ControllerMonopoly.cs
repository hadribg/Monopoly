...
namespace monopoly
{
	class ControllerMonopoly
	{
		private Plateau plateau;
		private vueMonopoly view;

		public ControllerMonopoly() {
		...
		public void run() {
			...
			Joueur joueurCourant = ControllerMonopoly.determinerPremier(this.plateau.getJoueurs());
			...
			while (this.plateau.getJoueurs().Count != 1) {
				...
				if (lesDoubles == 3) {
					...
					joueurCourant.setScore(plateau.getSommeDes());
				...
				Thread.Sleep (500);
				// Traiter le cas ou le joueur courant a atteri en prison suite à un double
				// Il ne rejoue pas
				if (lesDoubles == 0 || (joueurCourant.estEmprisonne() == 1)) {
